/// Test scheduler for the show controller's clock seam: records
/// scheduled callbacks and fires them when the virtual clock is
/// advanced. No real timers (docs/03 § 10.2.5).
final class FakeShowScheduler {
  final List<ScheduledShowCallback> _pending = [];

  /// Virtual time, milliseconds.
  int now = 0;

  /// Callbacks still scheduled (tests assert nothing lingers).
  List<ScheduledShowCallback> get pending => List.unmodifiable(_pending);

  /// Schedules [callback] at `now + [delay]`.
  void call(Duration delay, void Function() callback) {
    _pending.add(
      ScheduledShowCallback(
  ...
  /// schedule order; callbacks scheduling more work are picked up
...
final class ScheduledShowCallback {
  /// Creates the record.
  const ScheduledShowCallback({
    required this.dueMillis,
    required this.callback,
  });

  /// Virtual due time, milliseconds.
  final int dueMillis;
