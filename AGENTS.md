...
**Patrol (device E2E, docs/03 § 11)**: `patrol_cli` **3.11.0** (global: `dart pub global activate patrol_cli 3.11.0`) pairs with `patrol` dart package **3.20.0** (exact pin in `app/pubspec.yaml` — see the compatibility table before changing either). Run: `export PATH="$PATH:$HOME/.pub-cache/bin"` then `patrol test --device <serial> --target integration_test/<file>_test.dart`. `test_bundle.dart` is generated — never commit it.
