...
TongTong Royal is a Fall Guys-benchmarked show: qualification rounds 4→3→2→1, one crown, one-button controls, vs bots (multiplayer infra preserved for online shows). Full rules: `docs/01-game-design.md`.
...
**Never launch apps with `monkey`** (diagnosed 2026-09-19): on Samsung One UI, a `monkey -p <pkg> -c LAUNCHER` launch flips the device-global auto-rotate setting ON (persists after app exit — looks like "the app breaks my portrait lock"). Use `am start -n <pkg>/.MainActivity` everywhere.
...
4. **Feedback loop**: any NEW class of violation found gets added to a checklist in the relevant doc (same commit as the fix) so the gate catches it next time. Reviews MUST include a **layout-geometry pass**: for changed UI, check alignment/spacing defaults inherited from Material widgets (OverflowBar, ButtonBar, ListTile, dialog actions) — either overridden to a guide-pinned rule or explicitly accepted in the doc; silent defaults are findings (OverflowBar precedent 2026-09-19).
