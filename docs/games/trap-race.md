...
- **FINAL (2 players, quota 1)**: first finisher wins the **crown**. NO respawn — falling is elimination; if one finalist falls, the other wins by survival. Simultaneous final elimination (both fall on one tick) = shared crown (GDD § 7.2).
- **Round end**: the first instant the quota is met (GDD § 7.1). In the FINAL, a fall that leaves one alive ends the round instantly.
- **Timeout**: R1 — unfinished players rank by forward progress (furthest first); the quota fills in that order (progress ties share). FINAL —timeout ranks by progress; the leader takes the crown (shared on exact tie).
...
- Same-tick finishes share the qualifying slot; the next slot is skipped (v1 § 7.6 rule carried over).
